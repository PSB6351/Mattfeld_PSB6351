...
def infotodict(seqinfo):
    ...
    loc1_task = create_key('sub-{subject}/func/sub-{subject}_run-{item}_loc1_task')
    loc2_task = create_key('sub-{subject}/func/sub-{subject}_run-{item}_loc2_task')
    ...
    task_fmap = create_key('sub-{subject}/fmap/sub-{subject}_task-field_map')
    dwi_fmap = create_key('sub-{subject}/fmap/sub-{subject}_dwi-field_map')
    ...
    for s in seqinfo:
        ...
        if (slice_num == 176) and (timepoints == 1) and ("T1w_MPR_vNav" in s.series_description):
            info[t1w].append(s[2])
        elif (slice_num > 1) and (timepoints == 1) and ("fMRI_DistortionMap_PA" in s[12]):
            info[dwi].append(s[2])
        ...
        elif (timepoints == 355) and ("study1_task" in s[12]):
            info[study1_task].append(s[2])
        elif (timepoints == 351) and ('study2_task' in s[12]):
            info[study2_task].append(s[2])
        elif (timepoints == 350) and ('study3_task' in s[12]):                info[study3_task].append(s[2])
        elif (timepoints == 345) and ('study4_task' in s[12]):
